...
-- Materialized View on harmonized table 
CREATE OR REPLACE MATERIALIZED VIEW DAILY_ANALYTICS_MV AS
  SELECT DATA_DATE, VALUE 
  FROM  FRED_DB.FRED_HARMONIZED.FRED_HARMONIZED_TABLE ;


CREATE OR REPLACE MATERIALIZED VIEW WEEKLY_ANALYTICS_MV AS
  SELECT 
    DATE_TRUNC('WEEK', DATA_DATE) AS WEEK_START,
    MAX(DATA_DATE) AS WEEK_END,
